...
from .spectr_augment import SpecAugment, SpecCutout
...
class AudioToSpectrogramPreprocessor(AudioPreprocessor):
    ...
    def __init__(
        ...
        # self.featurizer = librosa.feature.melspectrogram(
        #     n_fft=self.n_fft,
        #     win_length=self.win_length,
        #     hop_length=self.hop_length,
        #     window_fn=window_fn,
...
class AudioToMFCCPreprocessor(AudioPreprocessor):
    ...
    def get_features(self, input_signal, length):
        ...
        return torch.from_numpy(np.expand_dims(mfcc, axis=0)).to(device)  
        #return self.featurizer(input_signal)
